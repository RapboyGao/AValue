...
class ALongitudeTests: XCTestCase {
    ...
    func testDifferentFormatIsSame() throws {
        // 6 + decimal digits
        XCTAssertEqual(try ALongitude("E0123456.7"), try ALongitude("E012°34'56.7\""))

        // 7 digits
        XCTAssertEqual(try ALongitude("E01234567"), try ALongitude("E012°34'56.7\""))

        // 6 digits
        XCTAssertEqual(try ALongitude("E0123456"), try ALongitude("E012°34'56\""))

        // 4 + deicimal digits
        XCTAssertEqual(try ALongitude("E01234.5"), try ALongitude("E012°34.5'"))

        // 5 digits
        XCTAssertEqual(try ALongitude("E012345"), try ALongitude("E012°34.5'"))

        // 4 digits
        XCTAssertEqual(try ALongitude("E01234"), try ALongitude("E012°34.0'"))

        // 3 digits (Faulty)

        // 2 digits
        XCTAssertEqual(try ALongitude("E012"), try ALongitude("E012°"))
    }
    ...
    func testInvalidInput() throws {
        ...
        XCTAssertNil(try? ALongitude("E039°16.5")) // 格式错误
    }

    func testNonStandardFormat() throws {
        XCTAssertNil(try? ALongitude("39°16'53.33\"E0")) // 方向符号在末尾
        XCTAssertNil(try? ALongitude("E039°16'53\".33")) // 错误的小数点位置
    }

    func testBoundaryValues() throws {
        let latitude1 = try ALongitude("E090°00'00\"")
        XCTAssertEqual(latitude1.toNumber(), 90.0)

        let latitude2 = try ALongitude("W090°00'00\"")
        XCTAssertEqual(latitude2.toNumber(), -90.0)

        let latitude3 = try ALongitude("E000°00'00\"")
        XCTAssertEqual(latitude3.toNumber(), 0.0)

        let latitude4 = try ALongitude("W000°00'00\"")
