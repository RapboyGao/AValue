...
public struct AWindLimit: Hashable, Sendable, Codable, CustomStringConvertible {
	...
	// 定义一个静态常量，表示B737的风限
	public static var b737 = AWindLimit(headWind: 50, tailWind: 10, crossWind: 30, totalWind: 100)
...
extension AWindLimit {
	...
	// 根据角度计算最大风限状态
	public func maxWind(from angle: AAngle) -> AWindLimitStatus {
	...
	// 返回一组代表不同方向的风限状态的数据
	public func usefulData() -> [AWindLimitStatus] {
	...
	// 将风速从一个单位转换到另一个单位
	public mutating func convert(from unit: AUnit, to newUnit: AUnit?) {
	...
	// 返回转换单位后的新风限
	public func converted(from unit: AUnit, to newUnit: AUnit?) -> AWindLimit {
		var copied = self
